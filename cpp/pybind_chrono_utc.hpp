...
// This is for casting times on the system clock into datetime.datetime instances
template<typename Duration>
class type_caster<std::chrono::time_point<std::chrono::utc_clock, Duration>>
{
public:
  using type = std::chrono::time_point<std::chrono::utc_clock, Duration>;
  bool load(handle src, bool)
  {
    using namespace std::chrono;
    ...
    value = time_point_cast<Duration>(utc_clock::from_time_t(std::mktime(&cal)) + msecs);
    return true;
  }

  static handle cast(const std::chrono::time_point<std::chrono::utc_clock, Duration> &src,
                     return_value_policy /* policy */, handle /* parent */)
  {
    using namespace std::chrono;
    ...
    std::time_t tt = utc_clock::to_time_t(time_point_cast<utc_clock::duration>(src - us));
    std::tm gmtm = *std::gmtime(&tt);
